...
- `pics/` 子目录 — 测试用图(输入图一律进 `pics/`,见 §1.1)

**代码归属不在文件名 / 目录结构上表达**,**在最终报告里明确指明** —— 哪几个函数 / 哪几行属于哪个 Task。报告里给每个 Task 一段"实现位于 `xxx.py` 的 `func()`"即可。

### 4.1 单文件长度 ≤ 400 行(2026-09-17 新增)

- **单个 `.py` 文件不超过 400 行**(含注释大致即可,超一点不致命,但别到 700)。
- 超了就按**职责**拆模块,**不要按 task 拆**。推荐切分:
  - `brick_geom.py` — 网格 / quadtree / region-merge / 三角形生成
  - `brick_color.py` — 调色板生成 + 量化(多色 / Otsu / K-Means / Median Cut)
  - `brick_render.py` — 三角形均值提取 + 渲染
  - `brick_metrics.py` — 评价指标(PSNR / SSIM / ΔE / Edge F1 / EPI / ...)
  - `brick_viz.py` — 所有绘图(尺寸直方图 / 调色板 / 对比图 / 热力图)
  - 各 Task 的 driver **保持薄**:只做参数解析、调度、结果落盘。
- 理由:长文件难读、难 review、AI 改起来容易误伤;按职责拆后 Task 4 可直接复用引擎模块,避免 Task 2/3/4 各抄一份。
...
### 5.7 代码注释三件套 (2026-09-18 新增)

任何函数 / 类 / 模块的 docstring 和关键代码块的内联注释,**必须**同时覆盖下列三项,缺一不可:

- [ ] **功能** —— 这段代码做什么(算法 / API 行为 / 业务逻辑,1-2 句话讲清)
- [ ] **形态** —— 涉及的 ndarray / tensor / 矩阵的 shape 变换写清:输入 shape、中间 shape、输出 shape
- [ ] **语义** —— 向量、矩阵、张量中**每个位置**的数值代表什么含义(例如:`[..., 0]=B` / `[i, 0]=x` / `labels[idx]=palette index`)

不满足三件套的注释视为"未注释",AI 必须补齐后再交付。简单的 1-2 行小函数可以三项融在 docstring 一段,但**形态 + 语义不能省**。
...
- 不要写超过 400 行的单文件(见 §4.1);超了就按职责拆模块
